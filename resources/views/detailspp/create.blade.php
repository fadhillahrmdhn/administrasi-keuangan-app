<x-app-layout>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 ">Tambah Data SPP</span>
                    </div>
                </li>
            </ol>
        </nav>
    ...
    <div class="flex justify-center items-center mt-10 container">
        <form class="max-w-xs w-full" action="/spp/detail" method="POST">
            @csrf
            <input hidden type="text" name="id" value="{{ $spp->id }}">
            <div class="mb-5">
                <label for="Selectbulan" class="block mb-2 text-sm font-medium text-gray-900">Bulan:</label>
                <select id="selectbulan" class="w-full" name="bulan" required
                oninvalid="this.setCustomValidity('Data tidak boleh kosong')" oninput="setCustomValidity('')">
                    <option value="">select...</option>
                    @foreach ($months as $month)
                    @if (is_null($spp->hargaSpp->$month))
            ...
            <div class="mb-5">
                ...
                <select id="selectharga" name="harga" class="w-full" required
                oninvalid="this.setCustomValidity('Data tidak boleh kosong')" oninput="setCustomValidity('')" />
                <option value="">select...</option>
                <option value="110000" {{ old('harga')=='110000' ? 'selected' : '' }}>Rp110.000</option>
                <option value="135000" {{ old('harga')=='135000' ? 'selected' : '' }}>Rp135.000</option>
                </select>
            </div>
